...
import { getWedding, getWeddingGuests } from "./utils/web3";
...
class Pixi extends Component<IPixiProps, IPixiState> {
  state: IPixiState = {
    wedding: null,
    guests: null,
    isLoaded: false
  ...
  async componentDidMount() {
    ...
    if (Number(weddingId) > 0) {
      const wedding = await getWedding(weddingId);
      const guests = await getWeddingGuests(wedding);

      guests.forEach(guest => {
        guest.tokenId = guest.tokenId.valueOf();

        // TODO: use CryptoKitties for now.
        guest.tokenContract = "0x06012c8cf97bead5deae237070f9587f8e7a266d";
      });

      // TODO: use CryptoKitties for now.
      wedding.tokenContractA = "0x06012c8cf97bead5deae237070f9587f8e7a266d";
      wedding.tokenContractB = "0x06012c8cf97bead5deae237070f9587f8e7a266d";

      wedding.tokenIdA = wedding.tokenIdA.valueOf();
      wedding.tokenIdB = wedding.tokenIdB.valueOf();

      this.setState({ isLoaded: true, wedding, guests });
    }
    ...
    this.setState({
      isLoaded: true,
      wedding: {
        tokenContractA: "0x06012c8cf97bead5deae237070f9587f8e7a266d",
        tokenContractB: "0x06012c8cf97bead5deae237070f9587f8e7a266d",
        tokenIdA: "1",
        tokenIdB: "1000000"
      },
      guests: [
  ...
  setup() {
    ...
    var ms = new PIXI.projection.Sprite2d(
    ...
    // create
    var u1 = new PIXI.projection.Sprite2d(
      PIXI.Texture.fromImage(
        `https://img.cryptokitties.co/${this.state.wedding.tokenContractA}/${
          this.state.wedding.tokenIdA
        }.png`
      )
    );
    ...
    var u2 = new PIXI.projection.Sprite2d(
      PIXI.Texture.fromImage(
        `https://img.cryptokitties.co/${this.state.wedding.tokenContractB}/${
          this.state.wedding.tokenIdB
        }.png`
      )
    );
    ...
    var guests = this.state.guests.map((guest: any) => {
      return (
    ...
    for (var i = 0; i < guests.length; i++) {
      ...
      guest.y = Math.floor(
        Math.random() * (this.app.screen.height / 2 - 100) +
  ...
  addPassiveInteraction(obj: any) {
    ...
    obj.on("pointerdown", this.onZoom);
  }
    ...
    return (
      <div>
        ...
        <p className="triangle-border top">
          I, ____, take you, ____, to be my lawfully wedded NFT, to have and to
